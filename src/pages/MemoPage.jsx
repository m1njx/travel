        ...
        <div className="flex gap-2">
          {memoText.trim() && (
            <motion.button
              ...
              className="p-2.5 rounded-xl bg-toss-bg text-toss-text-secondary hover:bg-toss-border/60 transition-colors btn-icon-sm"
              title="메모 전체 복사"
            >
          ...
          {!isEditing ? (
            ...
            >
              <Edit3 className="w-4 h-4" /> 메모 수정
            </motion.button>
          ) : (
            <motion.button
              whileTap={{ scale: 0.95 }}
              onClick={handleSave}
              className="flex items-center gap-1.5 px-4 py-2.5 bg-toss-success text-white rounded-xl text-[14px] font-semibold shadow-sm hover:bg-green-600"
            >
